...
public class WordPreprocessor : Preprocessor {
  private HandleRef swigCPtr;

  internal WordPreprocessor(IntPtr cPtr, bool cMemoryOwn) : base(modshogunPINVOKE.WordPreprocessorUpcast(cPtr), cMemoryOwn) {
  ...
  public override void Dispose() {
    lock(this) {
      if (swigCPtr.Handle != IntPtr.Zero) {
        if (swigCMemOwn) {
          swigCMemOwn = false;
          modshogunPINVOKE.delete_WordPreprocessor(swigCPtr);
        }
  ...
  public virtual short[] apply_to_feature_vector(short[] vector) {
		IntPtr ptr = modshogunPINVOKE.WordPreprocessor_apply_to_feature_vector(swigCPtr, vector.Length, vector);
    if (modshogunPINVOKE.SWIGPendingException.Pending) throw modshogunPINVOKE.SWIGPendingException.Retrieve();
		int[] size = new int[1];
